...
from base import timing, show_timing, PRINTING, cost_dict
...
class GAME(Gomokuy):
    def __init__(self, restricted=True, ai=False, difficulty=5):
        """
...
class GUI:
    def __init__(self, ui):
        ...
        self.bg = Canvas(ui, width=GUI_CONF["gap"] * 16, height=GUI_CONF["gap"] * 15 + GUI_CONF["flag"], bg="white")
        self.__bg_draw()
...
if __name__ == '__main__':
    window = Tk()
    gui = GUI(window)
    t1 = threading.Thread(target=GAME, args=(1,))
    t1.setDaemon = True
    t1.start()
    window.mainloop()
